...
Public Class frmAfficherListeRevenu
    ...
    Private Sub frmAfficherListeRevenu_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub proc_A()
        ...
        Try
            Dim myConnection As New OleDb.OleDbConnection(myConnString)
            myConnection.Open()
            Dim myCommand1 As New OleDb.OleDbCommand("Select sum(un) from check_revenu", myConnection)
    ...
    Sub proc_B()
        ...
        Try
            Dim myConnection As New OleDb.OleDbConnection(myConnString)
            myConnection.Open()
            Dim myCommand1 As New OleDb.OleDbCommand("Select sum(deux) from check_revenu", myConnection)
            Dim myReader1 As OleDb.OleDbDataReader = myCommand1.ExecuteReader()
    ...
    Sub proc_C()
        ...
        Try
            Dim myConnection As New OleDb.OleDbConnection(myConnString)
            myConnection.Open()
            Dim myCommand1 As New OleDb.OleDbCommand("Select sum(trois) from check_revenu", myConnection)
            Dim myReader1 As OleDb.OleDbDataReader = myCommand1.ExecuteReader()
    ...
    Sub proc_D()
        ...
        Try
            Dim myConnection As New OleDb.OleDbConnection(myConnString)
            myConnection.Open()
            Dim myCommand1 As New OleDb.OleDbCommand("Select sum(quatre) from check_revenu", myConnection)
            Dim myReader1 As OleDb.OleDbDataReader = myCommand1.ExecuteReader()
    ...
    Sub proc_E()
        ...
        Try
            ...
            Dim myConnection As New OleDb.OleDbConnection(myConnString)
            myConnection.Open()
            Dim myCommand1 As New OleDb.OleDbCommand("Select sum(cinq) from check_revenu", myConnection)
            Dim myReader1 As OleDb.OleDbDataReader = myCommand1.ExecuteReader()
    ...
    Private Sub ajouter_bouton()
        ...
        Label1.Location = New Point(430, y)
        Label2.Location = New Point(525, y)
        Label3.Location = New Point(675, y)
        Label4.Location = New Point(782, y)
        Label5.Location = New Point(935, y)
        Label7.Location = New Point(1060, y)
    ...
    Sub proc_Vider()
        ...
        Try
            ...
            ds = New DataSet
            tables = ds.Tables
            da = New OleDbDataAdapter("SELECT numero from check_revenu", myConnToAccess)
...
#Disable Warning BC42030 ' La variable 'numFour' est passée par référence avant qu'une valeur lui ait été attribuée. Cela peut provoquer une exception de référence null au moment de l'exécution.
                        obtenir_numRevenu(i.Text, numFour, date1, date2, montant)
#Enable Warning BC42030 ' La variable 'numFour' est passée par référence avant qu'une valeur lui ait été attribuée. Cela peut provoquer une exception de référence null au moment de l'exécution.
                        ...
                        supprimer_revenu(i.Text, numFour, date1, date2, montant)
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If (ListView1.SelectedItems.Count > 0) Then
            ...
            'frmUpdate_Paiement_preautorise.Show()
            Dim fc As New frmUpdate_Paiement_revenu()
            fc.TopLevel = False
